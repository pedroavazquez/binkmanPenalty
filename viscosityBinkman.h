/**
# Implicit viscosity solver with Brinkman penalization

This header extends Basilisk's standard `viscosity.h` by incorporating
the Brinkman penalization (BPM) term directly into the implicit
multigrid viscosity solve.

## Brinkman penalization inside the viscous step

The standard viscous diffusion equation for the velocity correction is:

    rho/dt * (u_new - u_old) = div(mu * (grad u + grad u^T))

The BPM extension adds a penalization body force:

    rho/dt * (u_new - u_old) = div(mu * (grad u + grad u^T))
                              - chi/eta * (u_new - us)

which, after temporal discretisation, introduces the term

    Kb = chi * dt / eta

on the diagonal of the linear system.  Because Kb >> 1 inside the
solid (eta is chosen small), the multigrid solve drives u_new → us
there, effectively enforcing the no-slip rigid-body condition.

## Declared fields

This header declares the three global Brinkman fields that the user
script must populate each timestep:

- `bpm_chi[]`  : solid volume fraction (0 outside, 1 inside)
- `bpm_eta[]`  : penalization parameter (m*Delta)^2/nu inside solid, 0 outside
- `bpm_us[]`   : prescribed solid velocity (Ucx, Ucy)

Both `residual_viscosity` and `relax_viscosity` are modified to include
these fields.  The `viscosity()` function restricts them to all grid
levels before calling the multigrid solver.

Used by: `centeredBinkman.h` → `binkmanViscosity.c`
...
/* Brinkman-penalization fields updated by the user script. */
scalar bpm_chi[];
scalar bpm_eta[];
vector bpm_us[];

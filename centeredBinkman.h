/**
# Incompressible Navier--Stokes solver with Brinkman penalization (centered formulation)

This header is a lightly modified copy of Basilisk's standard
`navier-stokes/centered.h`.  The only change is that it includes
`viscosityBinkman.h` instead of `viscosity.h`, so that the implicit
viscosity multigrid step also enforces the Brinkman penalization
condition inside the solid region.  An auxiliary centered vector field
`ubrinkman` is added to store the velocity after the viscous step
(before pressure projection), which can be used for diagnostics.

Used by: `binkmanViscosity.c`
See also: `viscosityBinkman.h` for the modified viscosity solver.
